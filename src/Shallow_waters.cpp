...
void Shallow_waters::assemble_stiffness_and_rhs_h(const double &time)
{
    ...
    const unsigned int n_q = quadrature_h->size();
...
void Shallow_waters::assemble_stiffness_and_rhs_u(const double &time)
{
    ...
    const unsigned int n_q = quadrature_u->size();
...
{
    pcout << "L2 norm " << 1e-6 * system_rhs.l2_norm() << std::endl;
    SolverControl solver_control(1000, 1e-6 * system_rhs.l2_norm());

    SolverCG<TrilinosWrappers::MPI::Vector> solver(solver_control);
    TrilinosWrappers::PreconditionILU preconditioner;
    preconditioner.initialize(
        lhs_matrix, TrilinosWrappers::PreconditionILU::AdditionalData());
...
{
    assemble_mass_matrix(fe_h.get(), dof_handler_h, quadrature_h.get(), mass_matrix_h);
    assemble_mass_matrix(fe_u.get(), dof_handler_u, quadrature_u.get(), mass_matrix_u);
    assemble_mass_matrix(fe_v.get(), dof_handler_v, quadrature_v.get(), mass_matrix_v);
    ...
    // Apply the initial condition.
    {
        ...
        VectorTools::interpolate(dof_handler_v, initial_conditions_v, solution_owned_v);
