...
directory_path = 'data/time/height'
data = compute_averages_in_directory(directory_path)

# Plot the results on a log-log scale
x = sorted(float(k) for k in data.keys())
y = [data[str(k)]/1e9 for k in x]

plt.figure()
plt.title("Convergence analysis")
plt.loglog(x, y, marker='o', linestyle='-', label='Measured average L2 error norm')
...
# y_ref2 = k2 / x^2
k2 = y[0] * (x[0]**2)  # pass through the first data point
y_ref2 = k2 / (np.array(x)**2)
plt.loglog(x, y_ref2, linestyle='--', color='green', label='Reference quadratic decrease')
